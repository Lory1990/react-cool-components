...
export function SingleSubscriptionCard(props: ISubscriptionCardProps) {
    const { htmlId, description, features, labels, price, mostConvenient, className, title, typology, paymentFrequency = 1, id, onBuyNowClick, disabled } = props
    ...
    return (
        <div
            className={classnames("subscription-card", className, style.subscriptionCard, { [style.mostConvenient + " most-convenient "]: mostConvenient, [style.disabled + " disabled"]: disabled })}
            id={htmlId}
        >
            ...
            {description && <div className={classnames(style.description, "description")}>{description}</div>}
            <div className={classnames(style.price, "price")}>
                <span className={classnames(style.priceValue, "price-value")}>
                    {price.currency} {priceString}
                </span>
                <span className={classnames(style.paymentFrequency, "payment-frequency")}>
                    {typology === "STANDARD" ? (paymentFrequency === 1 ? realLabels.toMonth : realLabels.toMonths.replace("{{number}}", paymentFrequency + "")) : ""}
                </span>
            </div>
            {onBuyNowClick && (
                <Button disabled={disabled} className={classnames(style.productPrice, "product-price")} onClick={e => onBuyNowClick(id || "")}>
                    {realLabels.buyNow}
                </Button>
            )}
                ...
                <>
                    ...
                    <ul className={classnames(style.listFeatures, "list-features")}>
                        {features.map((feature, index: number) => (
                            <li key={index} className={classnames(style.singleFeature, "single-feature")}>
                                {" "}
                                {feature.included ? <FeatureYes /> : <FeatureNo />} <span className={classnames("feature-text", style.featureText)}>{feature.text}</span>
                            </li>
                        ))}
                    </ul>
                </>
