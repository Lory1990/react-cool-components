...
export interface ISingleWorkCardProps {
    buttonText: string
    copy: string
    imageData: {
        imageUrl: string
...
export const SingleWorkCard: React.FC<ISingleWorkCardProps> = (props: ISingleWorkCardProps) => {
    ...
    return (
        <Card className={classNames(classes.root, style.card, "card")} onMouseOut={() => setCardStateHover()} onMouseOver={() => setCardStateHover()}>
            <figure>
                <div className={classNames(style["card__img-wrapper"], "card__img-wrapper")}>
                    <img className={classNames(style["card__img"], "card__img", `${cardImgAnimation}`)} src={props.imageData.imageUrl} alt={props.imageData.title} /> {/* END CARD IMG */}
                    {props.buttonText && props.link && (
                        <div className={classNames(style["card__img-overlay"], `${cardImgOverlayOn}`)}>
                            <div className={classNames(style["card__img-overlay-button"])}>
                                <a className={classNames(style["card__img-overlay-button-link"])} href={props.link}>
                                    {props.buttonText}
                                </a>
                            </div>
                        </div>
            ...
            <CardContent className={style["card__info"]}>
                ...
                {/* END CARD TITLE */}
                <Typography style={{ marginTop: 5 }} className={classNames(style["card__info-excerpt"])} color="textSecondary" component="p" variant="body2">
                    {props.copy}
                </Typography>{" "}
                {/* END CARD EXCERPT */}
            </CardContent>{" "}
