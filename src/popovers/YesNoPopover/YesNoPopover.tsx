...
export function YesNoPopover(props : IYesNoPopoverProps){

    let  { showCancel = true, showTitle = true, open = false, onClose, onSuccess, title, content, payload, closeLabel, okLabel, anchorEl, className,
        anchorOrigin = { vertical: 'bottom', horizontal: 'right',}, transformOrigin= {vertical: 'top', horizontal: 'right',}, okButtonProps, cancelButtonProps } = props
    ...
    >
            ...
            <div className={classnames(style.actions, 'actions')}>
                {showCancel && <Button className={classnames("cancel-button", style.cancelButton)}  variant='outlined' color='primary' onClick={onClose} {...okButtonProps}>{closeLabel || "Annulla"}</Button>}
                <Button className={classnames("ok-button", style.okButton)} color='primary' variant='contained' onClick={onSuccessInternal}  {...cancelButtonProps} >{okLabel || "Conferma"}</Button>
            </div>
    </Popover>
